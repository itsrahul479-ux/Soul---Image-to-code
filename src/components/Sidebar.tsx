...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const navItems = [
    {
      ...
      badge: sampleProjects.length,
      tooltip: `Projects (${sampleProjects.length})`,
    },
    {
      id: 'design' as const,
      label: 'Design System',
    ...
    },
    {
      ...
      tooltip: 'Prompt & AI History',
    },
  ];

  const handleNavClick = (panelId: 'design' | 'components' | 'assets' | 'history' | 'projects') => {
    setActivePanel(panelId);
    // If collapsed and user clicked an item, they might also want to expand it
  ...
  return (
    <aside
      className={`bg-[#0d121a] border-r border-[#1e2a3a] flex flex-col shrink-0 select-none z-20 transition-all duration-300 ease-in-out ${
        isCollapsed ? 'w-16' : 'w-72'
      }`}
    >
      {/* Sidebar Header with Toggle Button */}
      <div className="h-12 px-3 border-b border-[#1b2636] flex items-center justify-between shrink-0">
        {!isCollapsed ? (
          <>
            ...
            {onToggleCollapse && (
              <button
                ...
                className="p-1.5 rounded-lg text-slate-400 hover:text-sky-300 hover:bg-[#162130] border border-[#202d3e] hover:border-sky-500/40 transition-all flex items-center justify-center shadow-sm"
                title="Collapse sidebar (Ctrl+B)"
                aria-label="Collapse sidebar"
              >
                <PanelLeftClose className="w-4 h-4 text-slate-300 hover:text-sky-300" />
              </button>
            )}
          </>
        ) : (
      ...
      {/* Navigation item icons / tabs */}
      <div className="p-2 border-b border-[#1b2636] flex flex-col gap-1 shrink-0">
        {navItems.map((item) => {
          ...
          return (
            <button
              key={item.id}
              id={`panel-btn-${item.id}`}
              onClick={() => (isCollapsed ? handleNavClick(item.id) : setActivePanel(item.id))}
              className={`relative flex items-center rounded-xl text-xs font-semibold transition-all ${
                ...
                isActive
                  ...
                  : 'text-slate-400 hover:text-slate-200 hover:bg-[#162130] border border-transparent'
              }`}
              title={item.tooltip}
            >
              ...
              {!isCollapsed && typeof item.badge === 'number' && (
                <span className="text-[10px] font-mono font-bold bg-[#1a2536] px-1.5 py-0.5 rounded text-slate-400">
                  {item.badge}
                </span>
              )}

              {/* Indicator dot when collapsed and active */}
              {isCollapsed && isActive && (
                <span className="absolute right-1 top-1 w-1.5 h-1.5 rounded-full bg-sky-400 ring-2 ring-[#0d121a]" />
              )}
            </button>
          );
        })}
      ...
      {/* Main Panel Content Container (Expanded mode) */}
      {!isCollapsed ? (
        ...
        /* Collapsed Icon Rail Spacer */
        <div className="flex-1 flex flex-col justify-end p-2 items-center gap-2">
          {onOpenUpload && (
            <button
              onClick={onOpenUpload}
              className="p-2.5 rounded-xl bg-sky-500/10 hover:bg-sky-500/20 text-sky-400 border border-sky-500/30 transition-all shadow-sm"
              title="Upload Screenshot (New Project)"
            >
      ...
      {!isCollapsed && project && project.screens && project.screens.length > 0 && currentScreen && (
        <div className="p-3 border-t border-[#1b2636] shrink-0">
